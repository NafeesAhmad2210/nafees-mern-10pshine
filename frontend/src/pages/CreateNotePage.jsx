  ...
  return (
    <div className="p-8">
      <div className="max-w-2xl">
        ...
        <p className="text-slate-400 mb-6">Add a new note. Notes are saved to your account.</p>
        <button type="button" onClick={() => setModalOpen(true)} className="px-4 py-2 rounded-lg bg-amber-500 hover:bg-amber-600 text-slate-900 font-medium mb-6">New note</button>
        <h2 className="text-lg font-semibold text-slate-200 mb-3">3 most recent notes</h2>
        {loading && <p className="text-slate-400">Loading…</p>}
        {error && <p className="text-red-400 text-sm">{error}</p>}
        {!loading && !error && recentNotes.length === 0 && <p className="text-slate-400">No notes yet. Create one above.</p>}
        {!loading && recentNotes.length > 0 && (
          <ul className="space-y-3">
            {recentNotes.map((note) => (
              <NoteCard key={note._id} note={note} onToggleImportant={handleToggleImportant} onTrash={handleTrash} />
            ))}
          </ul>
        )}
      </div>
      <CreateNoteModal isOpen={modalOpen} onClose={() => setModalOpen(false)} onSave={handleSaveNote} />
    </div>
  );
}
